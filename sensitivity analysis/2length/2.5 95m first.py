...
dfx = pd.read_excel(f'C:\\Users\\User\\Desktop\\实验\\敏感性分析\\2借道长度\\{ppp}\\{ppp} sumo结果x{kkk}.xlsx')
dfv = pd.read_excel(f'C:\\Users\\User\\Desktop\\实验\\敏感性分析\\2借道长度\\{ppp}\\{ppp} sumo结果v{kkk}.xlsx')
...
for i in car:
    for t in (set(T[i]) & (set(range(31, 108)) | set(range(151, 228)))):
        if t == T[i][0]:
            continue
...
for i in range(2, len(car) + 1):
    for t in T[i]:
        ...
        elif t0[i-1] < t <= t0[i-1] + 12:
            m.addConstr(x[i-1, t] - x[i, t] - (dv + ds + v[i, t] * hs) >= 0)

for i in range(2, len(car) + 1):
    for ii in range(1, i):
        for t in T[i]:
            if (t > t0[i-1] + 12) and (t <= t0[ii-1] + aT):
                m.addConstr(x[ii, t] - x[i, t] - (dv + ds + v[i, t] * hs) >= -Q * e[i, ii, t])
                m.addConstr(o[i, ii, t] == (L[i, t] - L[ii, t]))
                m.addConstr(e[i, ii, t] == abs_(o[i, ii, t]))
...
if m.Status == GRB.OPTIMAL:
    ...
    x_result.to_excel(f"C:\\Users\\User\\Desktop\\实验\\敏感性分析\\2借道长度\\{ppp}\\{ppp} 初步x{kkk}.xlsx")
    L_result.to_excel(f"C:\\Users\\User\\Desktop\\实验\\敏感性分析\\2借道长度\\{ppp}\\{ppp} 初步L{kkk}.xlsx")
    a_result.to_excel(f"C:\\Users\\User\\Desktop\\实验\\敏感性分析\\2借道长度\\{ppp}\\{ppp} 初步a{kkk}.xlsx")
    v_result.to_excel(f"C:\\Users\\User\\Desktop\\实验\\敏感性分析\\2借道长度\\{ppp}\\{ppp} 初步v{kkk}.xlsx")