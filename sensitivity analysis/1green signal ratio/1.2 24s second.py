...
m.setObjective(quicksum((-x[i, t0[i-1]+aT])for i in car) + 0.1 * quicksum(a[i, t] ** 2 for i in car2 for t in T[i]) +
             1/96*quicksum(-x[i, t] for i in car1 for t in (set(T[i]) & (set(range(25, 121)) | set(range(145, 241))))),
               gp.GRB.MINIMIZE)
...
for i in car:
    for t in (set(T[i]) & (set(range(25, 98)) | set(range(145, 218)))):
        if t == T[i][0]:
            continue
...
for t in TT:
    if t <= 120:
        m.addConstr(Q * (G[t] - 1) - q <= 24 - t)  # t为30时，G绿1；t为120时，G红0 （不然车辆在120位置大于300）
        m.addConstr(24 - t <= Q * G[t] - q)
    elif (t > 120) and (t <= 240):
        m.addConstr(Q * (G[t] - 1) - q <= 144 - t)
        m.addConstr(144 - t <= Q * G[t] - q)
