...
dfL = pd.read_excel(f'C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\1.3\\1.3 初步L{kkk}.xlsx')
...
m.setObjective(quicksum((-x[i, t0[i-1]+aT])for i in car) + 0.1 * quicksum(a[i, t] ** 2 for i in car2 for t in T[i]) +
               1/90*quicksum(-x[i, t] for i in car1 for t in (set(T[i]) & (set(range(31, 121)) | set(range(151, 241))
                                      ))), gp.GRB.MINIMIZE)
...
if m.Status == GRB.OPTIMAL:
    ...
    x_result.to_excel(f"C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\1.3\\1.3 最终x{kkk}.xlsx")
    L_result.to_excel(f"C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\1.3\\1.3 最终L{kkk}.xlsx")
    a_result.to_excel(f"C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\1.3\\1.3 最终a{kkk}.xlsx")
    v_result.to_excel(f"C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\1.3\\1.3 最终v{kkk}.xlsx")