...
dfL = pd.read_excel(f'C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\{ppp}\\{ppp} 初步L{kkk}.xlsx')
...
m.setObjective(quicksum((-x[i, t0[i-1]+aT])for i in car) + 0.1 * quicksum(a[i, t] ** 2 for i in car2 for t in T[i]) +
             1/102*quicksum(-x[i, t] for i in car1 for t in (set(T[i]) & (set(range(19, 121)) | set(range(139, 241))))),
               gp.GRB.MINIMIZE)
...
for i in car:
    for t in (set(T[i]) & (set(range(19, 98)) | set(range(139, 218)))):
        if t == T[i][0]:
            continue
...
for t in TT:
    if t <= 120:
        m.addConstr(Q * (G[t] - 1) - q <= 18 - t)  # t为30时，G绿1；t为120时，G红0 （不然车辆在120位置大于300）
        m.addConstr(18 - t <= Q * G[t] - q)
    elif (t > 120) and (t <= 240):
        m.addConstr(Q * (G[t] - 1) - q <= 138 - t)
        m.addConstr(138 - t <= Q * G[t] - q)
...
if m.Status == GRB.OPTIMAL:
    ...
    x_result.to_excel(f"C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\{ppp}\\{ppp} 最终x{kkk}.xlsx")
    L_result.to_excel(f"C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\{ppp}\\{ppp} 最终L{kkk}.xlsx")
    a_result.to_excel(f"C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\{ppp}\\{ppp} 最终a{kkk}.xlsx")
    v_result.to_excel(f"C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\{ppp}\\{ppp} 最终v{kkk}.xlsx")