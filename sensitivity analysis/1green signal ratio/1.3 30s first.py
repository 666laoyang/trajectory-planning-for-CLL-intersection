...
'''dft = pd.read_excel(r'C:\\Users\\User\\Desktop\\实验\\sumo结果\t01.xlsx')
dfv = pd.read_excel(r'C:\\Users\\User\\Desktop\\实验\\sumo结果\\v01.xlsx')
v0 = dfv[0].tolist()  # car,t0,v0列表从0开始，如car[0]
t0 = dft[0].tolist()'''
kkk = 10
dfx = pd.read_excel(f'C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\1.3\\1.3 sumo结果x{kkk}.xlsx')
dfv = pd.read_excel(f'C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\1.3\\1.3 sumo结果v{kkk}.xlsx')
...
m.setObjective(1/90*quicksum(x[i, t] for i in car for t in (set(T[i]) & (set(range(31, 121)) | set(range(151, 241)))))
                         + quicksum((x[i, t0[i-1]+aT])for i in car), gp.GRB.MAXIMIZE)
...
for i in car:
    for t in (set(T[i]) & (set(range(31, 98)) | set(range(151, 218)) | set(range(271, 338)))):
        if t == T[i][0]:
            continue
...
if m.Status == GRB.OPTIMAL:
    ...
    x_result.to_excel(f"C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\1.3\\1.3 初步x{kkk}.xlsx")
    L_result.to_excel(f"C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\1.3\\1.3 初步L{kkk}.xlsx")
    a_result.to_excel(f"C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\1.3\\1.3 初步a{kkk}.xlsx")
    v_result.to_excel(f"C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\1.3\\1.3 初步v{kkk}.xlsx")