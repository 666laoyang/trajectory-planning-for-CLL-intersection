...
dfx = pd.read_excel(f'C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\{ppp}\\{ppp} {ggg}x{kkk}.xlsx')
dfv = pd.read_excel(f'C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\{ppp}\\{ppp} {ggg}v{kkk}.xlsx')
dfa = pd.read_excel(f'C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\{ppp}\\{ppp} {ggg}a{kkk}.xlsx')
dfL = pd.read_excel(f'C:\\Users\\User\\Desktop\\实验\\敏感性分析\\1绿信比\\{ppp}\\{ppp} {ggg}L{kkk}.xlsx')
...
for j in range(1, totalcar+1):
    for i in range(0, totaltime-120):
        if dfx.loc[i, j] == 0:
            xmax.append(dfx.loc[i+120, j])   # 找到车辆120s后的位置
            v0.append(dfv.loc[i, j])    # 找到车辆的初始速度
...
comfort = 0
for j in range(1, totalcar+1):
    comfort = comfort + sum(dfa.loc[i, j] ** 2 for i in range(t0[j-1], t0[j-1]+120))  # 最后1s的加速度不算
...
car2 = []
for j in range(1, totalcar+1):
    if dfL.loc[t0[j-1]+aT, j] == 1:
        car2.append(j)
print(len(car2)/totalcar)
'''-----------------------计算燃油-------------------------'''
fuel = 0
# 做k,c系数表
...
for j in range(1, totalcar+1):
    for i in range(t0[j-1], t0[j-1]+120):
        if dfa.loc[i, j] >= 0:
            fuel = fuel + math.exp(sum(k[j1][j2] * (dfv.loc[i, j]*3.6)**j1 * (dfa.loc[i, j]*3.6)**j2
                                       for j1 in range(0, 4) for j2 in range(0, 4)))
        elif dfa.loc[i, j] < 0:
            fuel = fuel + math.exp(sum(c[j1][j2] * (dfv.loc[i, j]*3.6)**j1 * (dfa.loc[i, j]*3.6)**j2 
                                       for j1 in range(0, 4) for j2 in range(0, 4)))
